    ...
    private void Update()
    {
        ...
        */
        ...
        if (!shootOnCD)
        {
            ...
            if (aimCountdown <= 0)
            {
                spriteAnimator.SetTrigger("AtkMain");
                _line.enabled = false;
                shootOnCD = true;
                LaunchProjectile();
                print("The if got called!");

                StartCoroutine("ShootCD");
            }

            //if the target is within range and is still aiming, update the path and decrement the aiming counter
            else if (Vector3.Distance(transform.position, target.transform.position) <= detectionRadius )//!shootOnCD)
            {
                spriteAnimator.SetTrigger("AtkStart");
                _line.enabled = true;
                DrawPath();
        ...
        else if (Vector3.Distance(transform.position, target.transform.position) > detectionRadius)
        {
            aimCountdown = aimTime;
            _line.enabled = false;
            spriteAnimator.SetTrigger("ReturnToIdle");
        }
        else if (shootOnCD)
        {
            aimCountdown = aimTime;
